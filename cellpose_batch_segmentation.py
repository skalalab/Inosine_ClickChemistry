...
if __name__ == "__main__":
    ...
    #%% Load Images
    path_dataset = Path(r"Z:\0-Projects and Experiments\GG - ClickChemistry")
    ...
    files = list(path_dataset.rglob("*.czi*"))
    ...
    # or in a loop
    for chan, filename in tqdm(zip(channels*len(files), files)):
        pass
        
        # img = load_image(filename) # tifffile.imread(filename)
        
        im_czi_data = czifile.imread(filename).squeeze()
        im_czi_data = xr.DataArray(im_czi_data, dims=('c','x','y'),
                                   coords={'c': ['toxo', 'dic', 'inosine', 'dapi']})
        
        img = im_czi_data.sel(c='inosine').data
